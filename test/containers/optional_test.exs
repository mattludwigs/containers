defmodule Containers.OptionalTest do
  ...
  setup_all do
    ...
    {:ok,
     %{
       hello_opt: Optional.to_optional(hello),
  ...
  test "implements Appendable", %{hello_opt: hello_opt, world_opt: world_opt, nil_opt: nil_opt} do
    ...
    assert %Optional{value: %Text{value: "Hello world!"}} =
             Containers.append(hello_opt, world_opt)
  end
  ...
  test "implements Mappable", %{hello_opt: hello_opt, nil_opt: nil_opt} do
    ...
    assert %Optional{value: "Hello!"} =
             Containers.map(hello_opt, fn %Text{value: t} -> t <> "!" end)
  end
  ...
  test "implements Joinable protocol correctly" do
    nested_nil_value = %Optional{value: %Optional{value: nil}}
    nested_value = %Optional{value: %Optional{value: 1}}

    assert Containers.join(nested_nil_value) == %Optional{value: nil}
    assert Containers.join(nested_value) == %Optional{value: 1}
  end
end
